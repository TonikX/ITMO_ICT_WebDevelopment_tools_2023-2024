...
from src.web_api.models import Book, BookCreate, BookModerationStatus, User
from typing import List, Union


def search_books(session: Session, query: str, offset: int, count: int) -> List[Book]:
    statement = (
        select(Book).where(Book.title.ilike(f"%{query}%")).offset(offset).limit(count)  # type: ignore
    )
