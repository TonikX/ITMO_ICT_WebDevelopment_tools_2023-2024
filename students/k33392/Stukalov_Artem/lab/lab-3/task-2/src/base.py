...
from .db import engine, engine_async, Book, BookModerationStatus
...
class WebParserBase(ABC):
    ...
    async def _save_db_async(self, value: BookDTO) -> None:
        async with AsyncSession(engine_async) as session:
            statement = select(Book).where(Book.title == value["title"])
            existing_book = (await session.exec(statement)).first()
            if existing_book:
                if self.log:
                    print(f'Trying to save existing book with title: {value["title"]}')
                return
