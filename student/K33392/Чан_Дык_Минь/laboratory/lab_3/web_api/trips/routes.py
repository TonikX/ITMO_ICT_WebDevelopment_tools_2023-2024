...
from url_parser.main import parse_url
...
# url: https://www.tsarvisit.com/en/visits/the-hermitage-408
# Parse url
...
# Trips endpoints
@router.get('/trip/all', tags=["trips"])
def trip_get_all(session=Depends(get_session)) -> List[Trip]:
...
def trip_delete(trip_id: UUID, session=Depends(get_session), current_user: User = Depends(auth_handler.get_current_user)):
...
def trip_update(trip_id: UUID, trip: TripDefault, 
...
def trip_requests_get(trip_id: UUID, 
                      session=Depends(get_session), 
...
def trip_requests_delete(trip_request_id: UUID, 
                         session=Depends(get_session), 
                         current_user: User = Depends(auth_handler.get_current_user),):
...
def trip_request_create(user_id: UUID, trip_request: TripRequestDefault, 
                        current_user: User = Depends(auth_handler.get_current_user), 
...
def update_trip_request_status(request_id: UUID, 
...
def add_user_to_trip(trip_id: UUID, 
                     user_id: UUID, 
                     session=Depends(get_session),
                     current_user: User = Depends(auth_handler.get_current_user)):
...
def remove_user_from_trip(trip_id: UUID, 
                          user_id: UUID, 
                          session=Depends(get_session),
                          current_user: User = Depends(auth_handler.get_current_user)):
